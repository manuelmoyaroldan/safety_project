...
export class CrashComponent implements OnInit {
    dialogRef: MdDialogRef<CrashDialog>;
    lastCloseResult: any;
    ...
    constructor(private _Service: CrashService, private viewContainerRef: ViewContainerRef, private dialog: MdDialog) { }
    ...
    open_Dialog(valueRec: any) {
        ...
        this.dialogRef = this.dialog.open(CrashDialog, config);
        this.dialogRef.componentInstance.current = JSON.parse(JSON.stringify(valueRec));

        this.dialogRef.afterClosed().subscribe(result => {
            if (result != undefined) {
                if (this.current.crashId != undefined) { //Edit
    ...
    click_Add(event: any) {
        ...
        //this.dialogRef = this.dialog.open(CrashDialog);
    ...
    click_Delete(valueRec: any) {
        this._Service
            .delete(valueRec)
            .then(() => {
                this.list = this.list.filter(h => h.crashId !== valueRec.crashId);
                this.selected = {};
            });
    }
    ...
    onSubmit() {
        if (this.selected.crashId != undefined) { //update
    ...
    delete() {
        this._Service
            ...
            .then(() => {
                this.list = this.list.filter(h => h.crashId !== this.selected.crashId);
                this.selected = {};
            });
    }
}
